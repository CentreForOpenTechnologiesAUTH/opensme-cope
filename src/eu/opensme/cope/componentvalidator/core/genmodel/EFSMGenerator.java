...
public class EFSMGenerator {
	...
	private final Collection<Class> imports = new TreeSet<Class>(
			new ImportClassComparator());
	// classes for which mock objects have been made
	private final Collection<Class> mocks = new HashSet<Class>();
	// map of methods and return values to create parameters for method calls
	// into SUT. key = method name, value = return type
	 ...
	 */
	public EFSMGenerator(Class objectUnderTest, FSMModel fsm, DaikonModel dm,
			Collection<Class> inputInterfaces,
			Collection<Class> outputInterfaces) {
		this.fsm = fsm;
		this.dm = dm;
		this.objectUnderTest = objectUnderTest;
		imports.add(objectUnderTest);
	 ...
	 * @param outputInterfaces
	 */
	public EFSMGenerator(Class objectUnderTest, String daikonOutput,
			String promOutput, Collection<Class> inputInterfaces,
			Collection<Class> outputInterfaces) {
		...
		FSMModel fsmTmp = prom.parse(in);
		DaikonModel dmTmp = daikon.parseAll();
		// since we only deal with the method names, we parseLexical those from
		// input/output interfaces to match against fsm states.
		// method name should equal stateid in fsm/daikon. later these are used
		// differentiate states as output or input
		this.fsm = fsmTmp;
		this.dm = dmTmp;
		this.objectUnderTest = objectUnderTest;
		imports.add(objectUnderTest);
	...
	//
	private void parseInterfaces(Collection<Class> inputInterfaces,
			Collection<Class> outputInterfaces) {
		this.outputInterfaces = outputInterfaces;
		Collection<String> inputs2 = InterfaceParser
				.methodNames(inputInterfaces);
		this.inputs.addAll(inputs2);
		mocks.addAll(outputInterfaces);
		// Collection<String> outputs =
		// InterfaceParser.methodNames(outputInterfaces);
	 ...
	 */
	public String generateEFSM(String packageName, String className, HashMap<String, String> statesHashMap) {
		Map<String, FSMState> fsmStates = fsm.getStates();
		StringBuffer body = new StringBuffer();
		for (String stateId : fsmStates.keySet()) {
			if (inputs.contains(stateId)) {
				body.append(ln);
				body.append(actionFor(stateId, statesHashMap));
				body.append(ln);
				body.append(guardFor(stateId));
			}
			// body.append(ln);
			body.append(transitionActionsFor(stateId, statesHashMap));
		}
		body.append(ln);
		body.append(valueMethods());

		StringBuilder efsm = new StringBuilder();
	...
	public String variables() {
		...
		for (Class clazz : mocks) {
			...
			variables += "  private " + className + " "
					+ mockNameFor(className) + ";" + ln + ln;
		}
		return variables;
	}
	...
	public String constructor(String className) {
		...
		String constructor = "  public " + className + "(" + parameter + ") {"
				+ ln + "    this." + objectName + " = " + objectName + ";" + ln
				+ "  }" + ln + ln;
		return constructor;
	}

	/**
	 * Generates the Java code to define a Collection/ArrayList for all the
	 * arrays present in the traces.
	 * 
	...
	/**
	 * Old code to create state enumeration. Not used anymore, just kept around
	 * for experimentation.
	 * 
	 * @param states
	 ...
	 * @return
	 */
	public String actionFor(String stateId, HashMap<String, String> statesHashMap) {
		if (!inputs.contains(stateId)) {
		...
		action += "    System.out.println(\"" + stateId.toUpperCase() + "\");"
				+ ln;
		action += sutCallWithNoOutputMethods(stateId, statesHashMap);
	...
	private String sutCallWithNoOutputMethods(String stateId, HashMap<String, String> statesHashMap) {
		String call = "";
		for (Class clazz : mocks) {
			MockGenerator mocker = new MockGenerator(clazz);
			call += mocker.replay();
		}
		call += sutCall(stateId, statesHashMap);
		for (Class clazz : mocks) {
			MockGenerator mocker = new MockGenerator(clazz);
			call += mocker.verify();
		}
...
/*        
	public String transitionActionsFor(String stateId, HashMap<String, String> statesHashMap) {
		if (!inputs.contains(stateId)) {
			return "";
		}
		...
		for (DaikonTransition transition : transitions) {
			...
			if (inputs.contains(target)) {
				// skip inputs here, they are to be their own action methods
				// with guards
				continue;
			}
			...
			actions += ln;
			actions += "  @Action" + ln;
			actions += "  public void " + stateId + "_" + target
					+ "() throws Exception {" + ln;
			String contents = "";
			contents += "this.state = \"" + asState(stateId) + "->"
					+ asState(target) + "\";" + ln;
			contents += "System.out.println(\"" + stateId.toUpperCase() + "->"
					+ target.toUpperCase() + "\");" + ln;
			...
			contents += sutCall(stateId, statesHashMap);
			contents += mocker.verify();
			contents = StringUtils.prefixWith(contents, "    ");
			actions += contents;
			actions += "  }" + ln + ln;
			actions += "  public boolean " + stateId + "_" + target
					+ "Guard() {" + ln;
...
*/
	 ...
	 * transitionMethods = ""; String result = ""; Collection<DaikonTransition>
	 * transitions = getTransitionsFrom(stateId); for (DaikonTransition
	 * transition : transitions) { String target = transition.getTarget(); if
	 * (inputs.contains(target)) { //skip inputs here, they are to be their own
	 * action methods with guards continue; }
	 * arrayNames.addAll(transition.arrayNamesForConstraints()); Method
	 * outputMethod = getOutputInterfaceMethodByName(target); Class
	 * outputInterface = outputMethod.getDeclaringClass(); MockGenerator mocker
	 * = new MockGenerator(outputInterface); String contents =
	 * "this.state += \"->"+asState(outputMethod.getName())+"\";"+ln; contents
	 * += mocker.callFor(outputMethod); contents += mocker.replay();
	 * mocks.add(outputInterface); contents += sutCall(stateId); contents +=
	 * mocker.verify(); contents += "return true;"+ln; contents =
	 * StringUtils.prefixWith(contents, "    "); String transitionMethodName =
	 * stateId + "_" + target + "_Transition"; String methodContent =
	 * "  public boolean "+transitionMethodName+"() {"+ln; methodContent +=
	 * transition.createTransitionGuards(); methodContent += ln+contents;
	 * methodContent += "  }"+ln; transitionMethods += ln+methodContent; result
	 * += "    if("+transitionMethodName+"()) return;"+ln; } return result; }
	 */
	public String assertFor(Method method, String returnVar) {
		...
		for (DaikonConstraint constraint : state.getConstraints()) {
			...
			// TODO to make this work, parseLexical object type from left and if
			// not string/primitive then make the call

			/*
			 * Collection<String> valuesToAdd =
			 * constraint.getAssertObjectValues(); if (valuesToAdd.size() == 0)
			 * { //avoid generating unnecessary methods continue; } String name
			 * = constraint.getAssertObjectName(); Collection<String> values =
			 * assertObjects.get(name); if (values == null) { values = new
			 * HashSet<String>(); assertObjects.put(name, values); }
	...
	/*
	 * public String returnValueBuilders() { String result = ""; for (String
	 * name : assertObjects.keySet()) { Collection<String> values =
	 * assertObjects.get(name); result +=
	 * "public "+name+" create"+name+"For(String str) {"+ln; for (String str :
	 * values) { result += "  if(str.equals("+str+")) {"+ln; result +=
	 * "    return null;"+ln; result += "  }"+ln; } result += "}"+ln; } return
	 * result; }
	 */
	 ...
	 * @return
	 */
	public String sutCall(String methodName, HashMap<String, String> statesHashMap) {
		Method method = getSUTMethod(methodName, statesHashMap);
		...
		String sutCall = objectName + "." + methodName + "(" + parameters
				+ ");" + ln;
	 ...
	 * @return
	 */
	private Method getSUTMethod(String methodName, HashMap<String, String> statesHashMap) {
		...
		return getOutputInterfaceMethodByName(methodName,statesHashMap);

/* changed by mokos                        
                throw new IllegalArgumentException("No method '" + methodName
				+ "' found in " + objectUnderTest + ".");
*/                
	}
	...
	public String valueMethods() {
		String result = "  //---------- TODO IMPLEMENT METHODS IN THIS SECTION TO GENERATE OBJECTS ------------"
				+ ln;
		...
		for (Iterator<String> i = valueMethods.keySet().iterator(); i.hasNext();) {
			...
			// the states are stored with actual method names, we must get the
			// original from the valuemethod name to access it
			 ...
			 * System.out.println("pindex:"+index); if (index == 2) {
			 * System.out.println("type:"+type.isPrimitive()); } }
			 */

			result += "  private " + type.getSimpleName() + " "
					+ parameterMethodName + " {" + ln;
	...
	public String minMaxJavaReturn(DaikonState state, int parameterIndex,
			Class type) {
		// TODO:state is is the method in general, not tied to transition in
		// which it is
		// executed. this could be narrowed.. or in spirit of some publications
		// it could also be extended to test with new values.. so its an open
		// option
		...
		if (diff < 0) {
			throw new IllegalStateException(
					"Invariant max is smaller than min for:"
							+ state.getFullName());
		}
	...
	public Method getOutputInterfaceMethodByName(String methodName) {
		...
		throw new IllegalArgumentException("No method '" + methodName
				+ "' found in any available output interface:"
				+ outputInterfaces);
	}
	...
	public String guardCheckMethods() {
		...
		if (true)
			return "";
		String methods = "//----------------------- GENERATED GUARD CHECK METHODS -----------------------------"
				+ ln;
		Map<String, FSMState> fsmStates = fsm.getStates();
	...
	/**
	 * Creates the basic Java code for the header part of the modeljunit EFSM
	 * code. This includes imports, class definitions, etc. All possibly used
	 * classes are always imported regardles, such as collection, arraylist, ...
	 * 
	...
	public String header(String packageName, String className) {
		...
		header += "import nz.ac.waikato.modeljunit.Action;" + ln;
		header += "import nz.ac.waikato.modeljunit.FsmModel;" + ln;
		header += "import nz.ac.waikato.modeljunit.Tester;" + ln;
//		header += "import nz.ac.waikato.modeljunit.RandomTester;" + ln;
                header += "import nz.ac.waikato.modeljunit.GreedyTester;" + ln;
		header += "import nz.ac.waikato.modeljunit.GraphListener;" + ln;
		header += "import nz.ac.waikato.modeljunit.coverage.ActionCoverage;"+ ln;
                header += "import nz.ac.waikato.modeljunit.coverage.CoverageMetric;"+ ln;
                header += "import nz.ac.waikato.modeljunit.coverage.StateCoverage;"+ ln;
                header += "import nz.ac.waikato.modeljunit.coverage.TransitionCoverage;"+ ln;
                header += "import nz.ac.waikato.modeljunit.coverage.TransitionPairCoverage;"+ ln
                + ln;
	...
	public String resetMethod(Collection<String> arrayNames,
			Collection<Class> mocks) {
		String reset = "  public void reset(boolean b) {" + ln;
		reset += "    state = \"\";" + ln;
		reset += "    System.out.println(\"------------------- STARTING TEST \"+testIndex+\"--------------------------\");"
				+ ln;
		reset += "    testIndex++;" + ln;
		...
		for (Class clazz : mocks) {
			reset += "    EasyMock.reset(" + mockNameFor(clazz.getSimpleName())
					+ ");" + ln;
		}
		reset += sutSetup();
		reset += "  }" + ln + ln;
...
//	}
		...
		String testMethod = "  @Test"
				...
				+ ln
				+ mockSetup()
				+ "    GreedyTester tester = new GreedyTester(this);"
				+ ln
				+ "    GraphListener listener = tester.buildGraph();"
				+ ln
				+ "    listener.printGraphDot(\""
				...
				+ "    CoverageMetric trCoverage = new TransitionCoverage();"
				+ ln
                                + "    CoverageMetric acCoverage = new ActionCoverage();"
				+ ln
                                + "    CoverageMetric trpCoverage = new TransitionPairCoverage();"
				+ ln
                                + "    tester.addListener(stCoverage);"
				+ ln
                                + "    tester.addListener(trCoverage);"
				+ ln
                                + "    tester.addListener(acCoverage);"
				+ ln
				+ "    tester.addListener(trpCoverage);"
				+ ln
				+ "    tester.addListener(\"verbose\");"
				+ ln
                                + "    //---------- DEFINE RANDOM TEST GENERATION NUM TO SUCCEED BETTER COVERAGE METRICS -----------------------------------------"
				+ ln
				+ "    tester.generate(20000);"
				+ ln
                                + "    tester.getModel().printMessage(stCoverage.getName() + \" was \" + stCoverage.toString());"
				+ ln
                                + "    tester.getModel().printMessage(trCoverage.getName() + \" was \" + trCoverage.toString());"
				+ ln
                                + "    tester.getModel().printMessage(acCoverage.getName() + \" was \" + acCoverage.toString());"
				+ ln
				+ "    tester.getModel().printMessage(trpCoverage.getName() + \" was \" + trpCoverage.toString());"
				+ ln + "  }" + ln + ln;
		return testMethod;
	}
	...
	private String helperMethods() {
		return ln
				...
				+ ln + numberHelper("int") + ln + numberHelper("float") + ln
				+ numberHelper("long") + ln + numberHelper("byte") + ln
				+ numberHelper("char") + ln + mainNumberHelpers() + ln
				+ randomCollectionItemHelper();
	}

	private String numberHelper(String type) {
		String methodName = capitalizeFirstLetter(type);
		return "  public " + type + " c" + methodName + "() {" + ln
				+ "    return (" + type + ") Math.round(cDouble());" + ln
				+ "  }" + ln + ln + "  public " + type + " c" + methodName
				+ "(" + type + " min, " + type + " max) {" + ln
				+ "    return (" + type + ") Math.round(cDouble(min, max));"
				+ ln + "  }" + ln;
	}
        
        private String mainMethod(){
            return "  public static void main(String[] args) {" + ln
				+ "    new org.junit.runner.JUnitCore().run(test.class);" + ln
				+ "  }" + ln;
        }

	private String mainNumberHelpers() {
		imports.add(Random.class);
		return "  public double cDouble() {" + ln
				+ "    double min = Integer.MIN_VALUE;" + ln
				+ "    double max = Integer.MAX_VALUE;" + ln
				+ "    return cDouble(min, max);" + ln + "  }" + ln + ln
	...
	public String generateEFSM(String packageName, String className,
			HashMap<String, String> statesHashMap,
			HashMap<String, String> variablesHashMap) {
		...
		efsm.append(helperMethods());
		efsm.append(guardCheckMethods());
                efsm.append(mainMethod());
		efsm.append(footer());
		return efsm.toString();

	}

	private Object transitionActionsFor(String stateId,
			HashMap<String, String> statesHashMap) {
		if (!inputs.contains(stateId)) {
			return "";
		}
		...
		for (DaikonTransition transition : transitions) {
			...
			arrayNames.addAll(transition.arrayNamesForConstraints());
			Method outputMethod = getOutputInterfaceMethodByName(target, statesHashMap);
			Class outputInterface = getMockClassFor(target, statesHashMap);
			MockGenerator mocker = new MockGenerator(outputInterface);
			actions += ln;
			actions += "  @Action" + ln;
			actions += "  public void " + stateId + "_" + target
	...
	private Class getMockClassFor(String methodName,
			HashMap<String, String> statesHashMap) {
		Set set = statesHashMap.entrySet();
		
		Iterator i = set.iterator(); 

		String hashedMethodName = methodName;
		while (i.hasNext()){
			Map.Entry<String, String> me = (Map.Entry<String, String>)i.next();
			if (me.getValue().equals(methodName)){
				hashedMethodName = me.getKey(); 
			}
		}
	...
	private Method getOutputInterfaceMethodByName(String methodName,
			HashMap<String, String> statesHashMap) {
