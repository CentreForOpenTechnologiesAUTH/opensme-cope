...
public class ClassVisitor extends org.apache.bcel.classfile.EmptyVisitor {
	...
	try {
	    cm.setDit(jc.getSuperClasses().length);
            ...
            String currentSubClass = visitedClass.getClassName();
            for (int i=0;i<superClasses.length;i++){
                String currentSuperClass = superClasses[i].getClassName();
                cm.addDITHierarchyDependency(currentSuperClass, currentSubClass);
                currentSubClass = currentSuperClass;
            }
	} catch( ClassNotFoundException ex) {
		String classNameNotFound = ex.getMessage().substring(ex.getMessage().lastIndexOf(" ",ex.getMessage().indexOf(":"))+1, ex.getMessage().indexOf(":"));
		cm.addDependencyNotFound(classNameNotFound);
	} 
