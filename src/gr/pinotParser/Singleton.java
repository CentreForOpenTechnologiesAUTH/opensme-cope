...
public class Singleton extends Pattern {
    ...
    public void addPatternBlock(ArrayList<String> block, long projectID) {
        for (int i = 0; i < block.size(); i++) {
            if (block.get(i).contains("is a Singleton class")) {
                try {
                    this.singleton = this.getValidClassFullName(
                            this.getClassName(block.get(i)), projectID);
                } catch (SQLException ex) {
                    Logger.getLogger(Singleton.class.getName()).log(Level.SEVERE, 
                            null, ex);
                }
                break;
            }
    ...
    public void writePatternToXml(Writer writer) throws IOException {
        ...
        writer.write("\t\t\t" + this.getParticipantWithXmlFormat("Singleton",
                this.singleton));
        writer.write("\t\t</instance>\n");
    }
    ...
    public void writePatternToCsv(Writer writer, String projectName) throws IOException {
        ...
        writer.write(";" + "Singleton" + ";" + this.singleton);
        writer.write("\n");
    }
